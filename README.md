...
### Notation for the Allocation Model
...
Here, $X_s$ is the main decision variable: it determines how many bikes should be placed at station $s$ at midnight before the next operating day begins. The variable $D$ is an auxiliary decision variable representing the largest imbalance across all stations $s \in \mathcal{S}$. Since the model minimizes $D$, the model tries to make the worst station as balanced as possible.
...
### Interpretation of the Decision Variables and Constraints
...
### Demand Estimation for the Net-Flow Input
...
### Role of the Allocation Decisions Inside the Discrete-Event Simulation
